...
const AnalyticsDashboard = ({ data = [] }) => {
    ...
    const stats = useMemo(() => {
        ...
        const systemLoad = Math.round((activeParked / 50) * 100); // Assuming 50 capacity
            ...
            <header className="max-w-7xl mx-auto mb-10 space-y-8">
                <div className="flex flex-col xl:flex-row justify-between items-start xl:items-center gap-6">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h2 className="text-2xl font-black text-white tracking-tight uppercase">System <span className="text-indigo-500">Intelligence</span></h2>
                            <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest">Real-time Data Processing</p>
                        </div>
                    </div>
                    ...
                    <div className="flex flex-wrap items-center gap-3 w-full xl:w-auto">
                        <div className="relative flex-1 md:w-56 group">
                            ...
                            <input
                                type="text"
                                placeholder="Vehicle ID..."
                                className="w-full bg-slate-900 border border-slate-800 rounded-xl py-2.5 pl-10 pr-4 text-xs text-white focus:border-indigo-500 outline-none"
                                value={searchTerm}
                                onChange={(e) => setSearchTerm(e.target.value)}
                            />
                        </div>
                        ...
                        <div className="flex bg-slate-900 border border-slate-800 rounded-xl p-1">
                            {['all', 'parked', 'departed'].map((status) => (
                                <button
                                    key={status}
                                    onClick={() => setFilterStatus(status)}
                                    className={`px-3 py-1.5 rounded-lg text-[10px] font-black uppercase transition-all ${filterStatus === status ? 'bg-indigo-600 text-white shadow-md' : 'text-slate-500 hover:text-slate-300'}`}
                                >
                                    {status}
                                </button>
                            ))}
                        ...
                        <select
                            className="bg-slate-900 border border-slate-800 rounded-xl py-2.5 px-4 text-[10px] font-black uppercase text-indigo-400 outline-none"
                            value={selectedZone}
                            onChange={(e) => setSelectedZone(e.target.value)}
                        >
                            <option value="all">Zones</option>
                        ...
                        <button onClick={exportToExcel} className="bg-emerald-600 hover:bg-emerald-500 text-white px-5 py-2.5 rounded-xl text-[10px] font-black flex items-center gap-2 transition-all shadow-lg shadow-emerald-600/20">
                            <Download size={14} /> EXCEL
                        </button>
                    </div>
                </div>

                {/* 2. STATS CARDS ROW (Unified Alignment) */}
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                    <StatCard title="Live Occupancy" value={stats.activeParked} icon={<Car size={20}/>} color="indigo" />
                    <StatCard title="Revenue Flow" value={`₹${stats.totalRevenue}`} icon={<Wallet size={20}/>} color="emerald" trend="+14%" />
                    <StatCard title="Total Traffic" value={stats.totalUnits} icon={<TrendingUp size={20}/>} color="blue" />
                    <StatCard title="System Load" value={`${stats.systemLoad}%`} icon={<Activity size={20}/>} color="amber" />
                </div>
            </header>
            ...
            <main className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <section className="lg:col-span-2 bg-slate-900/40 border border-slate-800/50 rounded-[2.5rem] overflow-hidden backdrop-blur-sm shadow-2xl">
                    <div className="p-6 border-b border-slate-800 flex items-center justify-between">
                        ...
                        <span className="text-[10px] font-bold text-slate-500 uppercase tracking-tighter">{filteredData.length} entries filtered</span>
                    </div>
                    ...
                    <div className="overflow-x-auto">
                        <table className="w-full text-left">
                            <thead>
                                <tr className="text-[10px] text-slate-600 uppercase tracking-widest bg-slate-950/50">
                                    ...
                                    <th className="px-6 py-4 font-black">Slot</th>
                                    <th className="px-6 py-4 font-black">Movement</th>
                                    <th className="px-6 py-4 font-black text-right pr-8">Fee</th>
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-slate-800/40">
                                {filteredData.map((session) => (
                                    <tr key={session._id} className="group hover:bg-indigo-600/[0.03] transition-all">
                                        ...
                                        <td className="px-6 py-5">
                                            <span className="bg-slate-950 border border-slate-800 text-indigo-400 px-3 py-1 rounded-lg text-[10px] font-black uppercase tracking-tighter">{session.slotNumber}</span>
                                        </td>
                                        <td className="px-6 py-5">
                                            <div className="flex flex-col gap-1.5">
                                                ...
                                                {session.exitTime ? (
                                                    <div className="flex items-center gap-2 text-[10px] text-slate-400 font-medium">
                                                        <LogOut size={10} className="text-rose-500" /> {formatFullDate(session.exitTime)}
                                                        <span className="text-[9px] text-indigo-500 font-black ml-1">({session.durationMinutes}m)</span>
                                                    </div>
                                                ) : (
                                                    <div className="flex items-center gap-1.5 ml-0.5">
                                                        <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse"></span>
                        ...
                        {filteredData.length === 0 && (
                            <div className="flex flex-col items-center justify-center py-20 text-slate-600">
                                <History size={48} className="opacity-10 mb-4" />
                                <p className="text-sm italic">No data match current filters</p>
                            </div>
                        )}
                    </div>
                </section>
                ...
                <aside className="space-y-8">
                    ...
                    <section className="bg-slate-900/40 border border-slate-800/50 rounded-[2.5rem] p-8 shadow-2xl">
                        ...
                        <div className="space-y-6">
                            {zones.map(zone => {
                                const zoneDataCount = data.filter(s => s.slotNumber?.startsWith(zone) && !s.exitTime).length;
                                const percentage = Math.min(zoneDataCount * 10, 100); 
                                return (
                                    <div key={zone} className="space-y-2">
                                        <div className="flex justify-between text-[10px] font-black uppercase tracking-widest">
                                            <span className="text-slate-500">Zone {zone}</span>
                                        ...
                                        <div className="h-1.5 w-full bg-slate-950 rounded-full overflow-hidden border border-slate-800">
                                            <div 
                                                className={`h-full transition-all duration-1000 ${percentage > 80 ? 'bg-rose-500' : 'bg-indigo-500'}`} 
                    ...
                    <div className="p-8 bg-indigo-600 rounded-[2.5rem] shadow-2xl shadow-indigo-600/20 relative overflow-hidden group">
                        <Zap className="absolute -right-4 -top-4 w-24 h-24 text-white/10 group-hover:scale-110 transition-transform duration-500" />
                        <h4 className="text-xs font-black text-white uppercase tracking-widest mb-6 border-b border-white/20 pb-2">Quick Insights</h4>
                        <div className="space-y-4">
                            <div className="flex justify-between items-end border-b border-white/10 pb-2">
                                <span className="text-[10px] text-white/70 font-bold uppercase tracking-widest">Total Income</span>
                                <span className="text-lg font-black text-white leading-none">₹{stats.totalRevenue}</span>
                            </div>
                            <div className="flex justify-between items-end border-b border-white/10 pb-2">
                                <span className="text-[10px] text-white/70 font-bold uppercase tracking-widest">Efficiency</span>
                                <span className="text-lg font-black text-white leading-none">High</span>
                            </div>
                        </div>
                        <p className="text-[10px] text-indigo-200 mt-6 leading-relaxed">
                            Turnover is optimal today. Average stay duration is trending within normal range.
                        </p>
                    </div>
                </aside>
            </main>
